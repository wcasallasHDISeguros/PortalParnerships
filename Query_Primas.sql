...
----Vista Materializada
CREATE MATERIALIZED VIEW gde_adp_ods.mv_polizas
AS
...
/* =====================================================================
   1. RED COMERCIAL VIGENTE
      ...
      IMPORTANTE:
      Se mantiene CURRENT_TIMESTAMP como primera opción.
      La MV se actualizará mediante REFRESH.
   ===================================================================== */
redcomercial_vigente AS (
    SELECT
        rc.cagente,
...
agentes_consulta AS (
    ...
    WHERE car.sproduc IN (
        900730,10024,900747,6031,6042,6041,6043,6044,
        6045,6046,6047,6048,6049,6032,6033,6034,6035,
        6038,6039,6024,6025,809,6023,6026,6027,6028,
        6029,6030,6052,7467,70106,8201,8202,8203,8204,
        8205,8206,8207,8208,8209,8210,8211,900748,10004,
        10011,900753,10012,10013,10014,10015,10016,
        10017,10018,10019,10003,6071,900731,900758,10020,
        10001,10000,900719,10021,10022,10023,111715,
        10002,7469,900745,900720,70107,900744,7452,807,
        808,10009,7468,900755,900759,900762,900774,
        900776,900775,900778,900777,900779,900771,
        900746,900742
    )
    AND car.ncertif = 0
),
...
/* =====================================================================
   7. F_NOMBRE - PERSONA NO PÚBLICA
      ...
      Por ahora se mantiene sin filtro por cagente de producción,
      exactamente como la versión que tenemos.
   ===================================================================== */
nombre_no_publico AS (
    SELECT
        db.cagente,
...
/* =====================================================================
   8. F_NOMBRE - PERSONA GENERAL
      ...
      Actualmente utilizamos el DUMMY que definimos.
   ===================================================================== */
nombre_persona AS (
    SELECT
        db.cagente,
...
/* =====================================================================
   9. TAPENOM
      ...
      Dummy temporal porque todavía no tenemos la tabla
      AXIS_PER_DETPER_CE.
   ===================================================================== */
tapenom AS (
    SELECT
...
/* =====================================================================
   12. PRIMA TOTAL POR CERTIFICADO
   ===================================================================== */
prima_certificado AS (
    SELECT
        s.sseguro,
